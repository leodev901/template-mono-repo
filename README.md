...
🚧 구조만 준비된 것 (미구현)
├── Legal Agent (agents/legal_agent/ - 빈 폴더)
...
flowchart TB
    ...
    Registry --> Weather & News
    News --> HttpClient --> NaverNews
    ChatService --> Agent --> Checkpoint & Summary
    Agent --> Models
...
│   │   ├── agents/                 # Agent 계층
│   │   │   ├── chat/
│   │   │   │   ├── prompts.py      # 시스템 프롬프트 + 역할별 프롬프트
│   │   │   │   └── toolset.py      # Chat Agent 허용 Tool 목록 + 필터
│   │   │   ├── legal_agent/        # (준비 중 - 빈 폴더)
│   │   │   └── tools/
│   │   │       ├── registry.py     # COMMON_TOOLS 카탈로그
│   │   │       ├── common/
...
│                              │ → 실제 Chat Agent 허용 목록(CHAT_TOOL_NAMES)과 불일치      │
├──────────────────────────────┼────────────────────────────────────────────────────────────┤
...
4. `/api/v1/tools` 반환을 Chat toolset 기준으로 정렬 또는 파라미터화
5. `user_role` 프롬프트 조합 활성화
